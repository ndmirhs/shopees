...
<br>
   <?php
    ...
    $sql = "SELECT * FROM customer_ship_summary";
    ...
    if ($result->num_rows > 0) {
        while ($rows = $result->fetch_assoc()) {
            ...
            echo "<button onclick=\"approveOrder('" . $rows['ORDER_ID'] . "')\">CANCEL</button>";
            echo "</div>";
...
<script>
    function approveOrder(orderId) {
        window.location.href = 'shiptry.php?orderId=' + orderId; // Redirect to cancel.php with orderId
    }
</script>
</body>
</html>
